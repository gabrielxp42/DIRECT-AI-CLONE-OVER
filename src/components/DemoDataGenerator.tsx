...
export const DemoDataGenerator = () => {
    ...
    const generateData = async () => {
        ...
        try {
            ...
            // 0.5 GARANTIR TIPOS DE PRODUTOS
            const typesConfig = [
            ...
            const addDemoOrder = (client: any, daysAgo: number, forceStatus?: string, isPaid = true) => {
                ...
                for (let k = 0; k < numItems; k++) {
                    ...
                    let qtx = (prod.type === 'DTF' || prod.type === 'Vinil' || prod.type === 'DTF UV') ? randomInt(3, 15) : randomInt(1, 10);
                    const subtotal = qtx * prod.price;
                    total += subtotal;

                    if (prod.name.includes('(Metro)') || prod.type.includes('DTF') || prod.type === 'Vinil') {
                        totalMetros += qtx;
            ...
            for (let i = 0; i < 25; i++) addDemoOrder(vipClients[0], i * 14 + randomInt(0, 5));
            for (let i = 0; i < 15; i++) addDemoOrder(vipClients[1], 35 + i * 20);
            ...
            const chunksData = <T,>(arr: T[], size: number): T[][] => Array.from({ length: Math.ceil(arr.length / size) }, (_, i) => arr.slice(i * size, i * size + size));

            const pedChunks = chunksData(pedidosBatch, 40);
