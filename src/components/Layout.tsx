...
const Layout = () => {
  ...
  return (
    <ModalQueueProvider>
      <div className="flex min-h-screen w-full transition-all duration-300 bg-background text-foreground overflow-hidden">
        {/* Sidebar - Desktop (Fixed) */}
        <div className={cn(
          "fixed left-0 top-0 bottom-0 hidden md:flex flex-col transition-all duration-300 liquid-glass border-r z-40",
          hideShell ? "-translate-x-full opacity-0 pointer-events-none" : "translate-x-0 opacity-100",
          !isExpanded ? "w-20" : "w-64"
        )} style={{ 
          borderColor: 'hsl(var(--primary) / var(--neon-border-opacity))',
          ...
          <div className="flex h-full flex-col gap-2">
            ...
            <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-4 overflow-hidden">
              ...
              >
                ...
                <svg width="32" height="20" viewBox="0 0 200 120" className="flex-shrink-0" style={{ filter: 'drop-shadow(var(--neon-shadow-sm))' }}>
                  <circle cx="50" cy="60" r="34" fill="none" stroke="currentColor" strokeWidth="8" className="text-primary-custom" opacity={showLauncher ? 1 : 0.8} />
                  <circle cx="150" cy="60" r="34" fill="none" stroke="currentColor" strokeWidth="8" className="text-primary-custom" opacity={showLauncher ? 1 : 0.8} />
                </svg>
                <span className={cn(
        ...
        <div className={cn(
          ...
          !hideShell && (isExpanded ? "md:pl-64" : "md:pl-20")
        )}>
          {/* Header (Fixed and overlapping Sidebar) */}
          <header className={cn(
            "fixed top-0 left-0 right-0 z-50 transition-all duration-300 liquid-glass",
            hideShell ? "translate-y-[-100%] opacity-0 pointer-events-none" : "translate-y-0 opacity-100"
          )} style={{ 
            borderColor: 'hsl(var(--primary) / var(--neon-border-opacity))'
          }}>
          ...
          <main className={cn(
            "relative flex flex-col flex-1 w-full overflow-x-hidden overflow-y-auto transition-all duration-300",
            !hideShell ? "pt-14 lg:pt-[60px]" : "pt-0",
        ...
        {!isOpen && !isMobile && !isFullScreenApp && (
          <button
            className="fixed bottom-4 right-4 h-16 w-16 rounded-full z-50 transition-all duration-300 hover:scale-110 active:scale-95 flex items-center justify-center overflow-hidden group"
