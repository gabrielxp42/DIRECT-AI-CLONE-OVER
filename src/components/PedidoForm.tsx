...
export const PedidoForm = ({ isOpen, onOpenChange, onSubmit, isSubmitting, clientes, produtos, initialData }: PedidoFormProps) => {
  ...
  const handleQuickClientSubmit = async (clientData: { nome: string; telefone?: string; email?: string; endereco?: string; valor_metro?: number }) => {
    ...
    try {
      ...
      showErrorVou corrigir completamente a lista de clientes no formulário de pedidos, garantindo que o scroll funcione corretamente. O problema está na implementação do componente CommandList que não está permitindo o scroll do mouse.

<dyad-write path="src/components/PedidoForm.tsx" description="Revisão completa da lista de clientes com scroll funcional">
"use client";
