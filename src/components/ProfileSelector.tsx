...
export const ProfileSelector: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 bg-black z-[100] flex flex-col overflow-hidden">
      {/* Intense Background Liquid Blobs */}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 max-w-5xl mx-auto">
              <AnimatePresence mode="popLayout">
                {subProfiles.map((sub, idx) => (
                  ...
                  >
                    <div className="relative aspect-square w-36 md:w-44 rounded-[3.2rem] overflow-hidden transition-all duration-300 shadow-[0_20px_40px_rgba(0,0,0,0.4)] bg-white/[0.04] backdrop-blur-[20px] border border-white/[0.08] flex items-center justify-center">
                ...
                {!isManagedMode && (
                  ...
                  >
                    <div className="aspect-square w-36 md:w-44 rounded-[3.2rem] border-2 border-dashed border-white/10 flex items-center justify-center group-hover:bg-white/[0.05] group-hover:border-primary/50 transition-all duration-300 backdrop-blur-[20px] bg-white/[0.02]">
                      <Plus size={40} className="text-zinc-600 group-hover:text-primary transition-all duration-300" />
                    </div>
            ...
            <div className="pt-10 flex flex-col items-center gap-6">
              <div className="relative flex flex-col md:flex-row items-center gap-4">
                <Button 
                  ...
                  className={cn(
                    "rounded-full px-12 h-16 border transition-all duration-300 group flex items-center gap-3 shadow-[0_20px_50px_rgba(0,0,0,0.3)] relative overflow-hidden",
                    isManagedMode 
                      ? "bg-white text-black border-white hover:bg-zinc-200" 
                      : "text-zinc-400 hover:text-white border-white/5 bg-white/[0.03] backdrop-blur-2xl hover:bg-primary/20 hover:text-primary"
                  )}
                >
                  <Settings2 className={cn("w-5 h-5", isManagedMode ? "text-black" : "text-zinc-500 group-hover:text-primary")} />
                ...
                {isManagedMode && (
                  <Button 
                    ...
                    className="rounded-full px-8 h-16 bg-zinc-900/80 text-white border border-white/10 backdrop-blur-3xl hover:bg-emerald-500 transition-all duration-500 flex items-center gap-3 uppercase font-black italic tracking-tighter"
                  >
      ...
      {createPortal(
        <AnimatePresence>
          {showPermissionsModal && tempPermissions && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center px-4 overflow-y-auto pt-20 pb-10">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.9, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 30 }} className="relative w-full max-w-4xl bg-zinc-900 border border-white/10 rounded-[3.5rem] p-8 md:p-12 shadow-[0_0_100px_rgba(0,0,0,0.5)] z-10 overflow-hidden">
                ...
                <div className="relative space-y-10">
                    ...
                    {(['atendente', 'designer', 'operador'] as UserRole[]).map((role) => (
                      <div key={role} className="space-y-6 bg-white/[0.02] border border-white/[0.05] rounded-[2.5rem] p-8">
                        <div className="flex items-center gap-4">
                          <div className="p-3 rounded-2xl bg-zinc-800 border border-white/10">{getRoleIcon(role)}</div>
                          <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {Object.entries(PERMISSION_LABELS).map(([key, info]) => (
                            <div key={key} onClick={() => togglePermission(role, key)} className={cn("flex items-center justify-between p-4 rounded-3xl border transition-all cursor-pointer group", tempPermissions[role]?.[key] ? "bg-emerald-500/10 border-emerald-500/30" : "bg-black/20 border-white/5")}>
                              <div className="flex flex-col">
                                <span className={cn("text-xs font-black uppercase tracking-tight", tempPermissions[role]?.[key] ? "text-emerald-400" : "text-zinc-400")}>{info.label}</span>
                                <span className="text-[10px] text-zinc-600 font-medium">{info.description}</span>
                              </div>
                              <div className={cn("w-10 h-6 rounded-full relative transition-colors", tempPermissions[role]?.[key] ? "bg-emerald-500" : "bg-zinc-800")}>
                                <div className={cn("absolute top-1 w-4 h-4 bg-white rounded-full transition-all", tempPermissions[role]?.[key] ? "left-5" : "left-1")} />
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="flex gap-4">
                    <Button variant="ghost" onClick={() => setShowPermissionsModal(false)} className="flex-1 h-16 rounded-3xl text-zinc-500 font-bold uppercase tracking-widest">Descartar</Button>
                    <Button disabled={savingPermissions} onClick={handleSavePermissions} className="flex-1 h-16 rounded-3xl bg-white text-black font-black uppercase italic tracking-tighter text-lg shadow-xl">
        ...
        <AnimatePresence>
          {showPinModal && (
            <div className="fixed inset-0 z-[10001] flex items-center justify-center px-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowPinModal(false)} className="fixed inset-0 bg-black/90 backdrop-blur-3xl" />
              <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-zinc-950/80 border border-white/10 p-10 rounded-[3rem] shadow-2xl z-10 text-center">
                <div className="flex flex-col items-center gap-6">
                  <div className="w-16 h-16 rounded-2xl bg-primary/20 flex items-center justify-center text-primary"><Lock size={32} /></div>
                  <div className="text-center space-y-1">
                    <h3 className="text-xl font-bold text-white">Insira seu PIN</h3>
                    <p className="text-zinc-500 text-sm">{pinPurpose === 'permissions' ? 'Ajuste de acessos é restrito ao Chefe' : `O perfil ${selectingProfile?.name} é protegido`}</p>
                  </div>
                  <input type="password" value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="••••" className="w-full bg-black/20 border border-white/5 rounded-2xl p-4 text-center text-3xl tracking-[1em] text-white outline-none focus:ring-2 focus:ring-primary/50" autoFocus onKeyDown={(e) => e.key === 'Enter' && handlePinSubmit()} />
                  <div className="w-full flex gap-3">
                    <Button variant="ghost" onClick={() => setShowPinModal(false)} className="flex-1 rounded-2xl h-12 text-zinc-500">Cancelar</Button>
                    <Button onClick={handlePinSubmit} className="flex-1 rounded-2xl h-12 bg-primary text-black font-bold">Confirmar</Button>
                  </div>
                </div>
              </motion.div>
            </div>
        ...
        <AnimatePresence>
          {showEditModal && editingSub && (
            <div className="fixed inset-0 z-[10002] flex items-center justify-center px-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowEditModal(false)} className="fixed inset-0 bg-black/80 backdrop-blur-2xl" />
              <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-xl bg-zinc-900 border border-white/10 p-10 rounded-[3.5rem] shadow-2xl z-10">
                <div className="space-y-8 relative z-10">
                  ...
                  <div className="flex gap-4 pt-4">
                    <Button variant="ghost" onClick={() => setShowEditModal(false)} className="flex-1 rounded-2xl h-14 text-zinc-400 font-bold">Voltar</Button>
      ...
      {createPortal(
        <AnimatePresence>
          {showCreateModal && (
            <div className="fixed inset-0 z-[10003] flex items-center justify-center px-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowCreateModal(false)} className="fixed inset-0 bg-black/80 backdrop-blur-2xl" />
              <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-xl bg-zinc-900 border border-white/10 p-10 rounded-[3.5rem] shadow-2xl z-10">
                <div className="space-y-8 relative z-10">
                  <div className="text-center space-y-2">
                  ...
                  <div className="flex gap-4 pt-4">
                    <Button variant="ghost" onClick={() => setShowCreateModal(false)} className="flex-1 rounded-2xl h-14 text-zinc-400 font-bold">Voltar</Button>
      ...
      {createPortal(
        <AnimatePresence>
          {showSetupPinModal && (
            <div className="fixed inset-0 z-[10004] flex items-center justify-center px-4">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-2xl" />
              <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-zinc-900 border border-white/10 p-10 rounded-[3.5rem] shadow-2xl z-10">
                <div className="space-y-8 relative z-10">
                  <div className="text-center space-y-2">
                    <div className="w-16 h-16 rounded-2xl bg-primary/20 flex items-center justify-center text-primary mx-auto mb-4"><ShieldCheck size={32} /></div>
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <input type="password" value={newPin} onChange={(e) => setNewPin(e.target.value)} placeholder="••••" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-center text-2xl tracking-[0.5em] text-white font-bold outline-none focus:ring-2 focus:ring-primary/50" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-black text-zinc-500 ml-4">Confirmar PIN</label>
                      <input type="password" value={confirmPin} onChange={(e) => setConfirmPin(e.target.value)} placeholder="••••" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-center text-2xl tracking-[0.5em] text-white font-bold outline-none focus:ring-2 focus:ring-primary/50" />
                    </div>
                  </div>
                  <div className="flex gap-4 pt-4">
                    <Button variant="ghost" onClick={() => { setShowSetupPinModal(false); setNewPin(''); setConfirmPin(''); }} className="flex-1 rounded-2xl h-14 text-zinc-400 font-bold">Voltar</Button>
                    <Button disabled={isSettingUp} onClick={handleSetupPinSubmit} className="flex-1 rounded-2xl h-14 bg-primary text-black font-black uppercase italic shadow-xl">{isSettingUp ? 'Salvando...' : 'Confirmar'}</Button>
                  </div>
                </div>
              </motion.div>
            </div>
