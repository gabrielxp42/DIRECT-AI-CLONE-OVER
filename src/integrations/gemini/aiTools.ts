...
export const callTool = async (functionCall: any) => {
  ...
  if (name === "list_orders") {
    ...
    if (!startDate && !endDate) {
      ...
      // If only startDate is provided, set endDate to the end of that day
      const start = new Date(startDate);
      start.setHours(0, 0, 0, 0); // Ensure start of day
      const end = new Date(start);
      end.setHours(23, 59, 59, 999);
      startDate = start.toISOString(); // Corrected: should be start of day
      endDate = end.toISOString();
    } else if (!startDate && endDate) {
      // If only endDate is provided, set startDate to the beginning of that day
