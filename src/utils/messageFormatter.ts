...
export const formatMessage = (content: string): string => {
    ...
    let formatted = content
        ...
        .replace(/(\d+[,.]?\d*\s?(ML|m\b))/gi, '<span class="text-yellow-500 font-bold">$1</span>')
        // Lists/Bullet points - Transform into clean rows with spacing
        .replace(/^\s*[-•]\s+(.*?)$/gm, '<div class="flex gap-2 items-start py-1 ml-1"><span class="text-yellow-500 font-black">•</span><span class="flex-1">$1</span></div>')
